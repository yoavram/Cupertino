...
Their analysis was specific to a model that classified individuals by the number of mutant genes in their genotype, similar to~\citet{Kimura1966} and~\citet{Haigh1978}. 
...
This is an extension of a model presented by~\citet{Altenberg2017}, allowing for the mutation rate of an individual $\mu_k$ to depend on the identity of the allele $A_k$.

Let the frequency of $A_k$ in the present generation be $f_k$ such that $f_k \ge 0$, $\sum_k{f_k}=1$, $\vec{f}=(f_1, \ldots, f_n)\tr$.
...
Here, we consider a large population of haploids in which individuals with genotype $A_k$ are characterized by the number of deleterious or mutant alleles in their genotype, $k$.
A modifier locus then determines the probability that a mutation occurs in individuals with $k$ mutant alleles, $m_k$.
In this case, if we assume a low mutation rate $m_k$ such that two mutations are unlikely to occur in the same individual in one generation.
...
This model has been presented and analyzed by~\citet{Ram2012}.
...
and
\begin{multline}
f_k' = (1 - m_k(\beta+\delta)) \frac{w_k}{\bar{w}} f_k + \\
